...
public class UserController {
    ...
    // 1. GET “/user/all”: This API allows the admin to fetch all the users from the
    // database
    ...
    // 2. POST "/user/register" (@RequestBody UserRequest userRequest): This API
    ...
    // 3. GET “/user/{id}”: This API allows the customer to fetch the user record by
    // its
    ...
    // 4. PUT "/user/{id}": This API allows customers to update a user record by its
    // ID and
    ...
    // 5. DELETE "/user/{id}" (@PathVariable Long id): This API lets the admin
    ...
    // 6. POST "/user/workout/{userId}" (@RequestBody WorkoutDto workoutDto,
    // @PathVariable Long userId): This API allows the trainer to assign a workout
    // to a customer by its
